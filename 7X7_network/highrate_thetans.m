function [Arms,Regret,Y,MaxUtil,ChosenUtil]=highrate_thetans(T,Ratings,ConstRealPool,RouteCost,RealTheta,poi)
...
while t<=T
    ...
    if RouteCost(d,t)<999   % Verify the most highly rated alternative
        ...
        X(t,3)=RouteCost(f,t);
        ChosenUtil(t,1)=[ConstRealPool(f,t),RealTheta(1,2:3)]*X(t,:)';
        Pr=(1+exp(-ChosenUtil(t,1)))^-1; 
        Regret(t,1)=MaxUtil(t,1)-ChosenUtil(t,1);
        Arms(t,1)=f;
